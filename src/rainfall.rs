...
pub fn mean(town: &str, strng: &str) -> f64 {
    match parse_strng(town, strng) {
        Some(temps) => temps.iter().sum::<f64>() / temps.len() as f64,
        None => -1.0
    }
}
...
fn parse_strng(town: &str, strng: &str) -> Option<Vec<f64>> {
    strng.split('\n')
        .filter_map(|x|
            if x.starts_with(town) {
                Some(x.split(':')
                    .collect::<Vec<&str>>()[1]
                    .split(',')
                    .map(|y| y[4..].parse::<f64>().unwrap())
                    .collect())
            } else {
                None
            }
